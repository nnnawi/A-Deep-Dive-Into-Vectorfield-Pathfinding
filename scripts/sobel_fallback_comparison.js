...
const sobel_fallback_comparison_sketch = (p, sketch_name) => {
    const arg = {
        screen: {
            w: 720,
            h: 280,
    ...
    function drawTitles() {
        ...
        const titles = [
            "Raw Sobel (No Fallback)",
            "Sobel with Same Distance Fallback",
            "Sobel with Min Neighbor Fallback"
        ];
    ...
    function dijkstra_optimized(node_grid, target, dist_arr, arg) {
        ...
        while (open_list.length > 0) {
            ...
            for (const [dx, dy] of neighbor_pos_arr) {
                ...
                if (nx < 0 || ny < 0 || nx >= arg.grid.w || ny >= arg.grid.h) continue;
        ...
        for (let y = 0; y < arg.grid.h; y++) {
            for (let x = 0; x < arg.grid.w; x++) {
                ...
                if (current_node.isWall) {
                    ...
                    gfx.fill(current_node.dist * 10, 255, 100);
                }
...
export default sobel_fallback_comparison_sketch;
