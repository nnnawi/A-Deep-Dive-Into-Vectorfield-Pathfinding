import { is_in_screen, is_in_grid, reset_node_grid } from './utils_function.js';
import { gfx_draw_grid, gfx_draw_heatmap } from './gfx_function.js';
import { djikstra_algorithm } from './solver_function.js';

class node {
    constructor(x, y){
        this.isWall = false;
        this.dist = 999;
        this.display_dist = 999;
        this.wasVisited = false;
        this.vector_angle = 0;
        this.display_vector_angle = 0;
        this.x = x;
        this.y = y;
    }
}

const vf_heatmap_distance_matrix_sketch = (p, sketch_name) => {
    const arg = {
        screen: {
            w: 720,
            h: 480,
        },
        
        grid : {
            tile_size: 30,
            w: 0,
            h: 0,
            stroke: 100,
            stroke_weight: 1,
        }
    }
    
    const dist_arr = {
        Tchebishev: [
            [2, 1, 2],
            [1, 0, 1],
            [2, 1, 2],
        ],
        Manhattan: [
            [1, 1, 1],
            [1, 0, 1],
            [1, 1, 1],
        ],
        Euclidian: [
            [1.41, 1, 1.41],
            [1, 0, 1],
            [1.41, 1, 1.41],
        ],
    }

    var node_grid;
    var current_distance = "Tchebishev"; // Default distance metric
    var dynamic_target_mode = false; // Track dynamic target mode state

    var is_dragging = false;
    var wallpen_state = true;

    var gfx_grid;
    var gfx_heatmap;

    var target = {
        x: 0,
        y: 0,
    }

    p.setup = () => {
        let canvas = p.createCanvas(arg.screen.w, arg.screen.h);
        canvas.parent(sketch_name);

        arg.grid.w = p.floor(arg.screen.w / arg.grid.tile_size);
        arg.grid.h = p.floor(arg.screen.h / arg.grid.tile_size);

        node_grid = new Array(arg.grid.h).fill().map((_,y) => new Array(arg.grid.w).fill().map((_,x) => new node(x, y)));

        gfx_grid = p.createGraphics(arg.screen.w, arg.screen.h);
        gfx_draw_grid(p, gfx_grid, arg);

        gfx_heatmap = p.createGraphics(arg.screen.w, arg.screen.h);
        djikstra_algorithm(node_grid, target, dist_arr[current_distance], arg);

        // Add event listener for radio buttons
        const radioButtons = document.querySelectorAll('input[name="distance"]');
        radioButtons.forEach(radio => {
            radio.addEventListener('change', (e) => {
                current_distance = e.target.value;
                reset_node_grid(node_grid);
                djikstra_algorithm(node_grid, target, dist_arr[current_distance], arg);
            });
        });

        // Add event listener for dynamic target toggle
        const dynamicTargetToggle = document.getElementById('dynamic-target-toggle');
        if (dynamicTargetToggle) {
            dynamicTargetToggle.addEventListener('change', (e) => {
                dynamic_target_mode = e.target.checked;
                if (!dynamic_target_mode) {
                    // Reset target position when disabling dynamic mode
                    target.x = 0;
                    target.y = 0;
                    reset_node_grid(node_grid);
                    djikstra_algorithm(node_grid, target, dist_arr[current_distance], arg);
                }
            });
        }
    }

    p.draw = () => {
        p.background(0);
        p.image(gfx_heatmap, 0, 0);
        p.image(gfx_grid, 0, 0);
        
        for(let y = 0; y < node_grid.length; y++){
            for(let x = 0; x < node_grid[0].length; x++){
                node_grid[y][x].update_display_dist(p, 0.05);
            }
        }   
    
        gfx_draw_heatmap(p, gfx_heatmap, node_grid, target, arg);
    }

    p.mouseMoved = () => {
        // Only respond to mouse events if mouse is over the canvas
        if (p.mouseX < 0 || p.mouseX > arg.screen.w || p.mouseY < 0 || p.mouseY > arg.screen.h) return;
        if (dynamic_target_mode && is_in_screen(p.mouseX, p.mouseY, arg)) {
            let grid_x = p.floor(p.mouseX / arg.grid.tile_size);
            let grid_y = p.floor(p.mouseY / arg.grid.tile_size);
            
            target.x = grid_x;
            target.y = grid_y;
            reset_node_grid(node_grid);
            djikstra_algorithm(node_grid, target, dist_arr[current_distance], arg);
        }
    }

    p.mouseDragged = () => {
        // Only respond to mouse events if mouse is over the canvas
        if (p.mouseX < 0 || p.mouseX > arg.screen.w || p.mouseY < 0 || p.mouseY > arg.screen.h) return;
        if (dynamic_target_mode) return; // Don't allow wall drawing in dynamic mode

        let x = p.floor(p.mouseX / arg.grid.tile_size);
        let y = p.floor(p.mouseY / arg.grid.tile_size);

        if(!is_in_grid(x, y, arg)) return;

        let current_node = node_grid[y][x];

        if(!is_dragging){
            wallpen_state = !current_node.isWall;
            is_dragging = true;
        }

        current_node.isWall = wallpen_state;
        reset_node_grid(node_grid);
        djikstra_algorithm(node_grid, target, dist_arr[current_distance], arg);
    }

    p.mouseReleased = () => {
        if(is_dragging) is_dragging = false;
    }

}

export default vf_heatmap_distance_matrix_sketch;