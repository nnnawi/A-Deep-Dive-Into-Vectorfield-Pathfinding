import { is_in_screen, reset_node_grid, is_in_grid } from './utils_function.js';
import { gfx_draw_grid, gfx_draw_heatmap, gfx_draw_arrow } from './gfx_function.js';
...
const weighted_neighbor_vf_sketch = (p, sketch_name) => {
    const arg = {
        screen: {
            w: 720,
            h: 480,
    ...
    p.setup = () => {
        ...
        const radioButtons = document.querySelectorAll('input[name="weight_function"]');
    ...
    // Update vector field using weighted neighbor sum method
    function update_weighted_neighbor_vectorfield(p, node_grid, arg) {
        for (let y = 0; y < arg.grid.h; y++) {
            for (let x = 0; x < arg.grid.w; x++) {
                ...
                // Compute weighted sum of neighbor vectors
                let vector = [0, 0];
                ...
                for (const neighbor_pos of neighbor_pos_arr) {
                    ...
                    // Handle boundaries and walls with fallback
                ...
                // Calculate angle (note: we subtract PI/2 to align with p5's coordinate system)
    ...
    // Draw vector field using weighted neighbor directions
...
export default weighted_neighbor_vf_sketch;
